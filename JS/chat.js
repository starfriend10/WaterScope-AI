...
// Shared model connection timer across Demo, Chat, and Agent
...
function startTimer(type = 'message') {
  ...
  timerStartTime = type === 'api-init' ? ensureSharedModelTimerStarted() : Date.now();
...
function updateTimerDisplay() {
  ...
  if (!el) return;
  const ms = timerStartTime ? Math.max(0, Date.now() - timerStartTime) : sharedModelElapsedMs();
  el.textContent = formatSharedModelElapsed(ms);
  el.style.display = 'block';
}
function stopTimer(options = {}) {
  if (timerInterval) clearInterval(timerInterval);
  timerInterval = null;
  if (options.completeModelConnection) completeSharedModelTimer();
  timerStartTime = null;
  updateTimerDisplay();
}
...
async function initializeGradioClient() {
    try {
        ...
        // Preserve completed model warm-up time for this browser tab.
        stopTimer({ completeModelConnection: true });
        return true;
    } catch (error) {
        ...
        // Preserve the original warm-up start after a failed connection.
        if (timerInterval) clearInterval(timerInterval);
        timerInterval = null;
        timerStartTime = null;
        updateTimerDisplay();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Restore shared model connection time across page navigation.
    updateTimerDisplay();
